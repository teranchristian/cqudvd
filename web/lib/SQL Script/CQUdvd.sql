...
CREATE TABLE ORDERS (   
   "ORDERS_ID"INTEGER NOT NULL GENERATED ALWAYS AS IDENTITY (START WITH 1, INCREMENT BY 1),   
    "MOVIES_BUY_ID" INTEGER,
    "MOVIES_RENT_ID" INTEGER,
    "PRICE" INTEGER,
    "USER_ID" INTEGER,
    CONSTRAINT primary_key_order PRIMARY KEY (ORDERS_ID),
    Foreign Key (MOVIES_BUY_ID) REFERENCES MOVIES_BUY (MOVIES_BUY_ID),
    Foreign Key (MOVIES_RENT_ID) REFERENCES MOVIES_RENT (MOVIE_RENT_ID),
